...
Pack `nextjs-auth@0.2.0` declares framework metadata `nextjs` with version range `>=15 <17`; callers select it with `framework: 'nextjs'`. **Since DG-10 (#254, ADR-008 Decision 9) the range is normative**: framework+version are detected from source evidence (pnpm-lock/package-lock/npm-shrinkwrap/yarn.lock lockfiles first, then package.json manifests, then imports), and a detected version outside the range blocks rule selection with `ARXIC-RULES-FRAMEWORK-REJECTED`. The range was widened from `>=15 <16` to `>=15 <17` (pack version `0.1.0` → `0.2.0`, widening = minor) with evidence: every rule in this pack is CI-verified against the reference app on `next@16.3.0` (`test-fixtures/reference-auth-app`, `real-world.test.ts` in both `@arxic/ast-grep-adapter` and `@arxic/orchestrator-langgraph`). The widening voids every waiver recorded against the old `>=15 <16` range — operators must re-record waivers against the current range. A deliberate operator override is a recorded waiver in the target repository's `arxic.waivers.json` (`ARXIC-RULES-FRAMEWORK-WAIVED`), never an implicit compatibility claim. All rules are original Arxic work, MIT licensed, with provenance `original-arxic`. App Router routes are connected by the deterministic `app/<feature>/page.tsx|route.ts` path convention; this path interpretation is not regex discovery.
