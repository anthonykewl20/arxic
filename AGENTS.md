...
- **Never write `close #<n>`, `fixes #<n>`, or `resolves #<n>` in a PR title or body unless you intend that issue to close on merge — including when you negate it.** GitHub's closing-keyword parser is not negation-aware: `does not close #27` contains `close #27` and closes it. This auto-closed the M1-EXIT gate issue twice, the second time from a PR whose own body said "do not close #27". Use `refs #<n>`, and if you need to be explicit, write "#<n> stays open".
