...
Captures are made reproducible before they are compared. Chromium runs with font hinting, subpixel positioning and LCD text off and its colour profile pinned to sRGB; an injected stylesheet forces every CSS animation and transition to its end state, because a context can only _ask_ a page to reduce motion and many pages ignore it; the wall clock is frozen so rendered dates and relative times repeat. `performance.now`, animation frames and the timers are deliberately left running — pinning them renders script-driven animation at its first frame while CSS sits at its last, and stalls any application that waits on a timer; script motion is settled by the existing re-capture loop instead. Readiness awaits image decoding as well as fonts.

A state checkpoint can also provoke the states plain navigation never reaches. It may answer the page's own data requests with a status from a closed list, so error banners, boundary fallbacks and toasts render, or submit the page's forms empty so inline validation appears. Both stay read-only against the target: the fault route never forwards the request it answers, and the capture context already aborts every non-GET. A checkpoint that answered no request, found no form or raised no surface records that as a finding rather than passing off an ordinary capture as an error state.

Overlays and portals are captured in isolation automatically — dialogs, alert dialogs, assertive live regions and toast containers — and a project can name component selectors to isolate alongside them. Each region becomes its own capture with its own baseline, so a component is compared against itself and a sibling's height change no longer reports it as altered. Regions are cropped from the already-masked page bytes, so an isolated capture can never disclose more than the page capture did.

When a capture differs from its baseline, the two runs' measured layout trees are compared to say what the difference _means_: content changed (something appeared or disappeared), the layout moved (an element changed position or size), or appearance only (every element kept its box, so the difference is paint). Alongside the changed-pixel count, structural similarity says whether the difference is spread thinly across the page — a tint or token change — or concentrated in a small area, two results a pixel count alone reports identically. Evidence that no longer hashes to what its capture recorded leaves the change unclassified rather than guessed; neither measure changes whether a capture counts as changed.

An approved baseline is stored under the digest of its own bytes rather than as a pointer into the run that produced it, so it survives that run's deletion, identical pixels are kept once, and `ARXIC_BASELINE_STORE` can place the store on durable or version-controlled storage.
