...
## 8. Remediation round 2 — delta re-review P3 residuals (final micro-round)

Delta re-review closed all 15 remediations; three P3 residuals fixed on the
same branch/surface (fourth commit), plus two accepted residuals documented.

| Residual                                         | Disposition           | Proof                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Accounting-gap record self-inconsistency         | FIXED                 | `finalizeLedgerArithmetic(before, cost, accountingGapCalls = 0)` freezes `after.remainingUsd` to 0 (true cumulative kept) — the extended gap test now proves the emitted record passes `validateRecordArithmetic` AND that the pre-fix non-gap-aware value is rejected ("forced-unparseable 200 … " test, negative control)                                                                                                                             |
| proxy.stop() pre-try window                      | FIXED (structural)    | `RecordingModelProxy.start()` moved to the LAST statement before the guarded region; outer finally also `proxy.stop()` (idempotent `#stopPromise`) so throws from `cloneBuildDigest`/`AttestationFront.start` can no longer strand the unref'd listener. No new test — simulating a throw from those internals mid-`runRealValidation` is not practical without refactoring the frozen surface; the structural fix is the deliverable (stated honestly) |
| koel row estimate stale (239 = DG-05 pre-fusion) | FIXED                 | `DG11_TARGET_ROW_ESTIMATES.koel = 306` (DG-06 fused `totalRows`, `docs/evidence/DG-06/fusion-summary.json`), README estimate-basis lines updated (koel: DG-06 fused; directus: DG-04 pre-fusion, fusion-time count unknown until first G-3 run). No committed refusal-proof artifact embeds koel-239-derived numbers — the only committed refusals are directus (`estimateUsd 0.0202368` = 272 × profile), verified; nothing to regenerate              |
| Validator single-zero-price hand-crafted record  | ACCEPTED (documented) | README § "Other accepted residuals" — unreachable from the runner (preflight refuses ANY zero price)                                                                                                                                                                                                                                                                                                                                                    |
| stop() #pending snapshot vs mid-stop request     | ACCEPTED (documented) | README § "Other accepted residuals" — unreachable (stop() runs after child exit under single-runner discipline)                                                                                                                                                                                                                                                                                                                                         |

Also in this round: README manual-repair procedure for accounting gaps now
covers RUN-RECORD reconciliation (reconstructed telemetry rows → measured
sums → ledger.after recompute) alongside the ledger entry, because the
validator cross-checks entry ↔ record ↔ telemetry in both directions.
