...
- WEB-DASHBOARD-REVAMP `searchRuns` takes a SET of project ids rather than one, so choosing an environment narrows Test runs the way it narrows every other screen. One id is the query it always was and an empty set narrows nothing — which is deliberately not the same as listing every id, because a run whose project has since been deleted matches none of them and the history is meant to outlive the project that produced it.
...
```
- WEB-DASHBOARD-REVAMP Clicking Pages while looking at one page did nothing: the guard only closed an open page on the way somewhere else, so the one control a person reaches for to get back to the grid was the one that did not work.
- WEB-DASHBOARD-REVAMP Choosing another project on Test runs left the previous project's run on screen until the server answered. The control the scope bar replaced painted the new scope first and fetched after; the bar waited for the round-trip, and a filter that appears not to have worked is one people press twice.
- WEB-DASHBOARD-REVAMP The two halves of the scope could contradict each other: choosing Production and then a development project left an empty screen to undo. The environment now narrows what the project switcher offers, in both directions.
- WEB-DASHBOARD-REVAMP The scope bar sat in the phone's collapsed header strip instead of travelling with the drawer, pushing every page down by 157px — far enough that a focused heading landed underneath it.
