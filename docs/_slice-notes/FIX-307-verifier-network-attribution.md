...
Issue: #307 · Status: per-request attribution fix implemented in this slice (F-E8), awaiting round-7 field proof · Disposition: observed

> **Round-6 amendment (2026-08-24, `directus-dg12-run6`):** the time-window rule recorded below was CONTRADICTED by the real target — directus's boot probe (`/auth/refresh` → 400 + console error) fires AFTER the first goto, inside the armed window, so stage-10 still blocked. The code that landed via #310 helps only the pre-navigation case. AC-4 failed. **This slice lands the corrected per-REQUEST rule (F-E8):** a step's goto opens a NAVIGATE window attributing only its DOCUMENT request; every generated action rides `enforceNetworkContainment`, which now opens an ACTION window attributing each request SENT while the awaited operation runs (marked at send time — late responses/failures still gate) plus console-error/pageerror raised inside the window; the app's boot probes are never marked and never gate; an ARMED suite that opens zero windows refuses at receipt write (fail-closed); unarmed suites (control-state specs) run transparently. Red-first: the F-E8 repro (a 404 subresource firing DURING the armed page load — the directus shape) and the updated contract test both fail against the pre-fix sources (`2 failed` / `1 failed`) and pass after. Remaining: round-7 field proof under #256.
