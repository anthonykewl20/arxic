...
- Make filtered declarations directly reachable by keyboard and pointer, including explicit zero-result navigation. Keep the heading and focus outline below the mobile header; wrap revision hashes at 320 pixels. Render every declaration of historical inventories whose persisted IDs duplicate, keying rows by immutable inventory position without rewriting saved evidence or requiring rediscovery. Extend the required installed reference-source/dashboard journey without removing any test file.
...
- `apps/web/src/__tests__/frontend-template.real-world.test.ts`: actual committed Express reference source, running page and source/installed dashboard, with unchanged thirteen-control assertion, plus a restored historical inventory carrying the pre-#460 duplicate ID format in a separate temporary database.
- Red: missing result navigation; first fragment-link implementation left the heading off-screen; 320-pixel hash overflow was 3 pixels; Firefox focus top was 63.5 against a 64-pixel minimum; historical duplicate IDs collided as React keys and retained 20 rows where 13 matched. Thresholds remain unchanged, viewport tightened from 390 to 320.
- Green: historical journey Chromium 13.93 s, Firefox 17.77 s, WebKit 15.56 s with the persisted historical inventory object-equal after viewing/filtering; workspace unit tests 5/5; full workbench light/dark regression 2/2 (105.31 s). Rendered rows key on immutable original-inventory position plus source ID — not filtered/page-relative index — with no dedupe, rewrite or rediscovery.
...
- No raw traces; named capture-masked screenshots and sanitized timeline/provenance are retained with red/green results in `docs/evidence/WEB-462-NAVIGATION/summary.md` and its hash manifest (172 files).
...
| Historical inventory has duplicate row IDs    | Every declaration renders without rediscovery; saved evidence unchanged | Real restored-database journey, three engines     |

Remaining: final browser and current-head installed CI, predecessor merge, integrator fold, historical duplicate-ID inventories beyond the reference source, wider themes/states and independent human release inspection. No completion claim.
