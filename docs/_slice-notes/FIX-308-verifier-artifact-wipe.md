...
(root cause detail in §5 evidence pointers)

## 3. `CHANGELOG.md` — entry under `## [Unreleased]` → `### fixed`

## 4. `VERSION` bump required?

no — verifier-internal staging fix; no shipped contract change.

## 5. Evidence pointers

### Root cause (the exact function)
...
| ----------------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| pre-existing foreign files under the caller's outputDirectory/artifacts | survive verify() untouched (observed)                                 | `#308 stage artifacts under outputDirectory/artifacts survive verification`          |
| trace sanitization failure inside the staged suite                      | blocked, no retained trace (observed)                                 | `blocks and retains no eligible trace when sanitization fails`                       |
| locked trace source inside the staged suite                             | blocked; eligible output removed (observed)                           | `blocks and removes eligible output when raw trace cleanup cannot unlink the source` |
| suite staging cannot be prepared                                        | blocked `ARXIC-VERIFY-SUITE-UNAVAILABLE` with the cause (fail-closed) | staging try/catch at verify() entry                                                  |

## 7. Not done / hand-off

### Fix design rationale (why isolated staging, not a narrower purge)
