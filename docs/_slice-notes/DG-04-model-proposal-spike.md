...
Issue: #248 · PR: #264 · Disposition: mixed (verified-by-CI for pipeline semantics; observed for real-model measurements — provisional pending cross-review per ADR-008 §11)
...
| Instruction-like model output (injection in rationale)         | blocked as content-is-data; frozen policy context unchanged                     | `sad-paths.test.ts`                       |
