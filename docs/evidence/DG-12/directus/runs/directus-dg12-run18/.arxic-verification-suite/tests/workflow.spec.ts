...
configureApprovedOrigins(["http://127.0.0.1:40477"]);

test("prop:d2869ca36472ed50", async ({ page, context }) => {
  await test.step("admin-page → admin-login-page", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:40477/admin/login"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:40477\/admin\/login(?:[?#].*)?$/);
