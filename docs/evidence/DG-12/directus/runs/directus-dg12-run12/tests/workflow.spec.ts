...
configureApprovedOrigins(["http://127.0.0.1:46097"]);

test("prop:56f20a9e1fdada43", async ({ page, context }) => {
  await test.step("admin-page → admin-login-page", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:46097/admin/login"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:46097\/admin\/login(?:[?#].*)?$/);
