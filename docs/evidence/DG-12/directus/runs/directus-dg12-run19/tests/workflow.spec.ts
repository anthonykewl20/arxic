...
configureApprovedOrigins(["http://127.0.0.1:36895"]);

test("prop:766ef534236e8526", async ({ page, context }) => {
  await test.step("admin-page → admin-login-page", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:36895/admin/login"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:36895\/admin\/login(?:[?#].*)?$/);
