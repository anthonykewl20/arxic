...
configureApprovedOrigins(["http://127.0.0.1:19503"]);

test("prop:b02617f1f9382f7e", async ({ page, context }) => {
  await test.step("admin-page → admin-login-page", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:19503/admin/login"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:19503\/admin\/login(?:[?#].*)?$/);
