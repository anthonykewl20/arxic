...
configureApprovedOrigins(["http://127.0.0.1:43259"]);

test("prop:3e87733f06dba070", async ({ page, context }) => {
  await test.step("admin-page → admin-login-page", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:43259/admin/login"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:43259\/admin\/login(?:[?#].*)?$/);
