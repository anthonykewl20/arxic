...
configureApprovedOrigins(["http://127.0.0.1:33745","http://127.0.0.1:8123","https://app.lemonsqueezy.com"]);

test("prop:754355e865abdd2f", async ({ page, context }) => {
  await test.step("home → home", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:33745/#/home"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:33745\/(?:[?#].*)?$/);
