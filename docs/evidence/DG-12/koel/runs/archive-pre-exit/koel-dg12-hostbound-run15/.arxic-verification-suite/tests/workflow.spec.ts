...
configureApprovedOrigins(["http://127.0.0.1:28507","http://127.0.0.1:8123","https://app.lemonsqueezy.com","https://assets.lemonsqueezy.com"]);

test("prop:c051b253cd3aa8c3", async ({ page, context }) => {
  await test.step("home → home", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:28507/#/home"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:28507\/(?:[?#].*)?$/);
