...
configureApprovedOrigins(["http://127.0.0.1:24767","http://127.0.0.1:8123","https://app.lemonsqueezy.com","https://assets.lemonsqueezy.com","https://www.gravatar.com"]);

test("prop:0791be5412c44a68", async ({ page, context }) => {
  await test.step("home → home", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:24767/#/home"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:24767\/(?:[?#].*)?$/);
