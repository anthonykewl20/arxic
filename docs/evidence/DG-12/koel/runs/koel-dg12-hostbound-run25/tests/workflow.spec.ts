...
configureApprovedOrigins(["http://127.0.0.1:25815","http://127.0.0.1:8123","https://app.lemonsqueezy.com","https://assets.lemonsqueezy.com","https://www.gravatar.com"]);

test("prop:08ab5475ccc5ff5e", async ({ page, context }) => {
  await test.step("home → home", async () => {
    armReceiptCapture(page);
    await withReceiptAttribution(page, 'navigate', () => page.goto("http://127.0.0.1:25815/#/home"));
    ...
    assertNetworkContained(context);
    await expect(page).toHaveURL(/^http:\/\/127\.0\.0\.1:25815\/(?:[?#].*)?$/);
