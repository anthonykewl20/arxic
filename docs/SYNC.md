...
**Status:** **Milestone 1 IN PROGRESS (3/15).** #17 (M1-03) ships `@arxic/orchestrator-langgraph`, the M1 control-plane `StateGraph` for pipeline stages 0–12. It composes the real stage 0/1/3/5/12 adapters, reuses Source-UA's normalized structural output at stage 2, and keeps stages 4/6/7/8/9/10 injectable for their later owner slices; stage 11 is a recorded M2-deferred no-op. Immutable SHA-256 artifact refs cross nodes. File and memory checkpointers persist per-stage run state; reconstructing a worker resumes from the last completed checkpoint and re-runs at most the active stage. Status flows `queued` → `running` → optional `awaiting-approval` → `completed|partial|failed`; malformed model output retries then blocks without promotion, empty candidates are `observed`/`partial`, hash or redaction mismatch fails closed, and blocked adapter diagnostics suppress promotion. Real-world proof reconstructs during stage 4 against the real `reference-auth-app`, then runs real Crawlee/Chromium without recomputing stages 0–3. Frozen §10 contracts remain unchanged; only stage 10 may originate `verified`.

**Next action:** [#18 (M1-04) Candidate reconciliation + coverage matrix](https://github.com/anthonykewl20/arxic/issues/18) — replace the stage-6 deterministic reconciliation seam with the real reconciler and classify static/runtime conflicts without changing the orchestrator graph.

**Last session:** 2026-08-05 (12) — **#17 (M1-03) LangGraph orchestrator DONE**: `@arxic/orchestrator-langgraph` owns the stage 0–12 `RunState`, status/failure classification, immutable artifact refs, per-stage observability, approval pause, and file/in-memory checkpoints. Real adapters back attestation, Source-UA, ast-grep, Crawlee, and promotion; later M1 owners replace typed seams without graph edits. Stable `ARXIC-ORCH-*` diagnostics validate through frozen contracts. Real reference-app proof reconstructs after a stage-4 worker loss, reuses stages 0–3, runs real Crawlee/Chromium, retries malformed output, suppresses promotion, and proves prompt bytes absent from persisted records. Frozen §10 contracts unchanged; stage 10 is the only `verified` origin. Gates green (typecheck/lint/format/license/test, 292 tests). **M1 3/15.** Next: #18 M1-04.
