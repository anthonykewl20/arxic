...
**Status:** **Milestone 1 IN PROGRESS (5/15).** #19 (M1-05) ships real persona, inbox, and OTP `FixtureProvider` service blocks plus the stage-7 `FixtureCoordinator` action. Required capabilities provision opaque disposable leases; missing providers, prior-run lease leakage, secret/PII exposure, missing or ambiguous Mailpit messages, and targets without the app-supported seed API fail closed as `blocked` with stable `ARXIC-FIXTURE-*` diagnostics. The coordinator redacts candidate requirements before stage artifacts persist and resets/releases leaked state rather than fabricating fixtures. Real reference-app seed/reset, SMTP→Mailpit reset mail, and otplib TOTP proof leaves the persona and inbox empty after release. Stage 7 now carries leases with `provisioned: true`; LangGraph topology is unchanged.

**Next action:** [#20 (M1-06) Playwright compiler: IR → plan + spec](https://github.com/anthonykewl20/arxic/issues/20) — compile evidence-backed Workflow IR into policy-safe human-readable plans and independent Playwright specs.

**Last session:** 2026-08-06 — **#19 (M1-05) Fixture adapters (persona/inbox/otp) DONE**: `OtpAdapter`, Mailpit-backed `InboxAdapter`, and app-seed-only `PersonaProvisioner` implement the frozen `FixtureProvider`; the stage-7 `FixtureCoordinator` maps required preconditions, provisions redacted opaque leases, detects/reset/releases leakage, and blocks missing or unsafe fixture paths without fabrication. Optional Testcontainers Mailpit startup is isolated in `@arxic/environment`. Real reference-app seed/reset, reset-email, Mailpit API, and otplib proof leaves no persona or inbox leakage. Frozen §10 contracts and orchestrator topology unchanged. Gates green (typecheck/lint/format/license/test, 322 tests). **M1 5/15.** Next: #20 M1-06.
