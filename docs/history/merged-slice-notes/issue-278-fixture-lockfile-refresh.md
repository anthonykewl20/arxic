...
Issue: #278 · PR: #279 · Disposition: verified
...
P2 evidence-artifact refresh — scope extension DECISION posted+ratified on #278 (comment 5327784471): `docs/evidence/DG-10/matrix.json` regenerated by the recorded harness invocation against the refreshed fixture (16.2.6→16.2.11, lockfile blobSha256 `8d0775bd…`→`094212ea…`, cross-checked against the fixture sha256; `koel-detection.json` content-identical — koel-only inputs untouched; re-running the recorded invocation 2026-08-18 reproduced `matrix.json` byte-identically and `koel-detection.json` byte-identically after the repo format pass, raw output differing only in `sources`-array layout, which prettier (printWidth 100) collapses back to the committed form) + `docs/evidence/DG-10/README.md` provenance lines updated; P3 harness coherence guard — `measure-framework-gate.mts` now asserts manifestPin === lockfileResolution before building the matrix (mirrors `expectedCampaignNextVersion()` in framework-gate.test.ts; proved red-first: planted 16.2.6-manifest/16.2.11-lockfile aborts with a clear error before writing any artifact, exit 1); independent review verdict APPROVE-WITH-P2-P3 → both addressed.
