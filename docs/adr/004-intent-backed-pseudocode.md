...
- Replacing `defaultCompile` with the full compiler generator — **resolved (M2-COMPILE-DEFAULT, PR #149)**: `defaultCompile` now wraps the full `PlaywrightCompiler.compile` via a try/catch adapter (populates `stagedBundle`; throws become `{compiled:false}`); the loose fallback generator is removed, so unsupported assertions and missing-evidence candidates are blocked instead of loosely accepted.
