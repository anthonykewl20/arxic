...
- Replacing `defaultCompile` with the full compiler generator — **resolved (M2-COMPILE-DEFAULT, PR #149)**: stage 9's uninjected path now adapts the policy-gated `PlaywrightCompiler.compile`, retains the full plan and staged bundle, and converts safe structured compiler diagnostics to blocked compilation results.
