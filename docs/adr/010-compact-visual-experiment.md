...
A deterministic reproduction plus feature-lane ablation ([failure analysis](../evidence/VISUAL-SLM/failure-analysis/summary.md), 2026-09-06) attributed the miss to single-application training/calibration and per-application geometry fingerprints rather than an implementation defect; the current-clip feature alone transferred perfectly, reinforcing that the deterministic check stays preferable for that criterion. The original three-application holdout is henceforth development data; generalization claims need a fresh untouched holdout.
...
Production adoption requires the remaining [specification](../visual-small-model-spec.md) gates: richer independent data, proven incremental value, resource qualification and release evidence. Per the owner directive of 2026-09-06 (spec §17 addendum), GLM Flash continues implementation engineering directly under the unchanged rules; the pre-teacher gate still governs any future teaching experiment. This experiment may validly conclude that deterministic checks are preferable for a class.
