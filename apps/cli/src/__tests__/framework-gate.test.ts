...
function configYaml(frameworks: string[], repository: string): string {
  return `version: 1
source:
...
async function scenario(options: {
  ...
  await writeFile(join(directory, 'arxic.yaml'), configYaml(options.frameworks, directory), 'utf8');
...
describe('DG-10 CLI config gate: unknown and out-of-range frameworks fail fast', () => {
  it('frameworks: [laravel] exits 2 before any crawl, with a path-free unknown-framework diagnostic', async () => {
    const outcome = await scenario({ frameworks: ['laravel'] });
