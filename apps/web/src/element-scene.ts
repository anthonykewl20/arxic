...
export type ElementScene = Pick<VisualScene, 'viewport' | 'nodes' | 'truncated'>;
...
/** Numeric-only, image-bound projection for inspection; no new solver verdicts. */
export function parseElementScene(
  ...
  for (const entry of scene.nodes) {
    ...
    if (
      ...
      return;
    ids.add(n.id);
    nodes.push({ id: n.id, parent: n.parent, x: n.x, y: n.y, width: n.width, height: n.height });
  }
  return {
