...
export async function startWebCommand() {
  ...
  if (
    !Array.isArray(roots) ||
    !roots.length ||
    !roots.every((value) => typeof value === 'string' && isAbsolute(value))
  )
    throw new Error('ARXIC_WEB_ROOTS must be a nonempty JSON array of absolute folder paths');
  const port = Number(process.env.ARXIC_WEB_PORT ?? '4310');
  if (!Number.isInteger(port) || port < 0 || port > 65535)
    throw new Error('Invalid ARXIC_WEB_PORT');
