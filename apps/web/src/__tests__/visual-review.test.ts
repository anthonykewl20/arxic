...
it.each([false, true])(
  'requires inspected pixels, preserves source evidence and blocks ungrounded AI regions (matrix=%s)',
  async (matrix) => {
    ...
    try {
      const project = await wb.saveProject({
        ...
        captureConsent: true,
        ...(matrix
          ? { browsers: ['chromium', 'firefox', 'webkit'], colorSchemes: ['light', 'dark'] }
          : {}),
      });
      const run = wb.enqueue(project.id, 'visual');
      await wb.idle();
      const capture = wb.store.run(run.id)!.result!.captures!.at(-1)!;
      if (matrix) expect(capture.environment).toEqual({ browser: 'webkit', colorScheme: 'dark' });
      const input = {
      ...
      expect(result.review).toMatchObject({
        ...
        findings: [
          { truthState: 'hypothesized', region: { x: 8, y: 100, width: 700, height: 250 } },
        ],
      });
...
it.each([
  ...
  ).rejects.toThrow('Capture integrity check failed');
});
