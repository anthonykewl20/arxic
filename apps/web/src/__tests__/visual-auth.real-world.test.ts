...
it('blocks missing sign-in secrets, then captures a real authenticated Next.js page without retaining session state or raw video', async () => {
  ...
  try {
    ...
    const evidence = process.env.ARXIC_AUTH_CAPTURE_EVIDENCE_DIR;
    if (evidence) {
