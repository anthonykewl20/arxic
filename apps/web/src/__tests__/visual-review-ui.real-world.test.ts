...
it.each([1, 2] as const)(
  ...
  async (density) => {
    ...
    const evidence = process.env.ARXIC_REVIEW_EVIDENCE_DIR
      ? density === 1
        ? process.env.ARXIC_REVIEW_EVIDENCE_DIR
        : join(process.env.ARXIC_REVIEW_EVIDENCE_DIR, 'density-2x')
      : undefined;
    ...
    try {
      ...
      if (density === 2) {
        ...
        await page.getByLabel('2× sharp', { exact: true }).check();
      }
      ...
      expect(await page.locator('.review-image svg').getAttribute('viewBox')).toBe(
        density === 2 ? '0 0 1600 1200' : '0 0 800 600',
      );
      ...
      await proof(
        '03-mobile-review',
        'Review image and findings remain within the mobile viewport',
      );
      ...
      expect(
        await page.getByLabel('I inspected this screenshot', { exact: false }).isChecked(),
      ).toBe(false);
      await page.locator('.review-controls').scrollIntoViewIfNeeded();
      await proof(
