...
900-second command limit, 25-minute job limit and per-test assertions remain unchanged. Installed Firefox/WebKit CI uses two required exhaustive partitions of the same sixteen files; this increases aggregate execution capacity per browser without omitting or duplicating a file. Default local and packed Chromium runs still execute all sixteen files.
